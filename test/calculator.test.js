...
import { createCalculationState, formatKoreanMoney } from '../src/ui-state.js';
...
test('ancillary cost subtotal exposes the requested total',()=>{assert.equal(calculate({...base,price:700_000_000}).ancillaryCostTotal,14_508_590)});
test('calculation only runs when explicitly triggered and changes mark the result stale',()=>{let calls=0;const state=createCalculationState(values=>{calls++;return values.price});assert.equal(state.hasResult,false);state.markChanged();assert.equal(calls,0);assert.equal(state.dirty,false);assert.equal(state.run({price:700_000_000}),700_000_000);assert.equal(calls,1);state.markChanged();assert.equal(calls,1);assert.equal(state.result,700_000_000);assert.equal(state.dirty,true);assert.equal(state.run({price:708_000_000}),708_000_000);assert.equal(calls,2);assert.equal(state.dirty,false)});
test('Korean money helper always formats the current input value',()=>{assert.equal(formatKoreanMoney(700_000_000),'약 7억원');assert.equal(formatKoreanMoney(708_000_000),'약 7억 800만원');assert.equal(formatKoreanMoney(1_200_000_000),'약 12억원');assert.equal(formatKoreanMoney(1_243_500_000),'약 12억 4,350만원')});
