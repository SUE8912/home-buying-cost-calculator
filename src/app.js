import { calculate, formatKoreanAmount } from './calculator.js';
...
const inputs = ['purchase-price', 'loan', 'paid', 'bond'];
let current;

function syncInput(el) {
  const raw = el.value.replace(/[^0-9]/g, ''); el.value = raw ? Number(raw).toLocaleString('ko-KR') : '';
  const helperId = el.getAttribute('aria-describedby') || `${el.id}-korean`;
  const helper = $(`#${helperId}`); if (helper) helper.textContent = formatKoreanAmount(Number(raw));
}

function getValues() {
  return { price: parse($('#purchase-price')), firstHome: $('[name=firstHome]:checked').value === 'yes', over85: $('[name=area]:checked').value === 'over', loan: parse($('#loan')), paid: parse($('#paid')), bond: $('#bond').value.trim() ? parse($('#bond')) : null };
}

const rows = r => [
  ['tax', '취득 관련 세금', '취득세 · 지방교육세 · 농어촌특별세', won(r.acquisitionTotal), 'mint'],
  ['broker', '중개보수 상한', '실제 보수는 상한 내 협의 · 부가세 별도', won(r.brokerage), 'blue'],
  ['legal', '법무사 기본보수 예상', '대한법무사협회 보수기준에 따른 간이 추정', won(r.legal), 'beige', '예상'],
...
function openDetail(type) {
  ...
  if(type==='legal') d={title:'법무사 기본보수 예상',amount:r.legal,badge:'예상',conditions:`매매가를 간이 기준금액으로 사용 · ${won(r.price)}`,standard:'대한법무사협회 보수기준에 따른 구간별 기본보수',formula:`매매가 기준 자동 계산 ${won(r.legal)}`,facts:[['법무사 기본보수 예상',won(r.legal)]],why:'대한법무사협회 보수기준에 따른 기본보수 예상액입니다. 본 계산기는 입력한 매매가를 기준금액으로 사용한 간이 추정이며, 실제 비용은 과세표준액, 가산보수, 대행업무, 실비 및 부가세 등에 따라 달라질 수 있습니다.'};
