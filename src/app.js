...
import { createCalculationState, formatKoreanMoney } from './ui-state.js';

const $ = s => document.querySelector(s);
const won = n => `${Math.round(n).toLocaleString('ko-KR')}원`;
const shortWon = n => formatKoreanMoney(n).replace(/^약 /, '');
const parse = el => Number(el.value.replace(/[^0-9]/g, '')) || 0;
const inputs = ['purchase-price', 'loan', 'paid', 'legal', 'bond'];
const calculationState = createCalculationState(calculate);
let current = null;

function syncInput(el) {
  const raw = el.value.replace(/[^0-9]/g, ''); el.value = raw ? Number(raw).toLocaleString('ko-KR') : '';
  const helper = $(`#${el.id}-korean`); if (helper) helper.textContent = formatKoreanMoney(Number(raw));
}
...
function render() {
  current = calculationState.run(getValues()); const r = current;
  ...
  $('#ancillary-subtotal').textContent = won(r.ancillaryCostTotal);
...
function markChanged() {
  calculationState.markChanged();
  if (calculationState.dirty) $('#result-status').textContent = '입력값이 변경되었어요. 다시 계산해주세요.';
}
...
inputs.forEach(id => { const el=$(`#${id}`); el.addEventListener('input',()=>{syncInput(el); markChanged();}); });
document.querySelectorAll('input[type=radio]').forEach(el=>el.addEventListener('change',markChanged));
document.querySelectorAll('.chips').forEach(group=>group.addEventListener('click',e=>{if(!e.target.dataset.value)return; const el=$(`#${group.dataset.target}`);el.value=Number(e.target.dataset.value).toLocaleString();group.querySelectorAll('button').forEach(x=>x.classList.toggle('active',x===e.target));syncInput(el);markChanged();}));
$('#calculator-form').addEventListener('submit',e=>e.preventDefault());
$('#calculate-button').addEventListener('click',()=>{render();$('#results').scrollIntoView({behavior:'smooth'});});
