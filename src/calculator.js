...
export function calculate({ price, firstHome, over85, loan, paid, legal = 800_000, bond = null }) {
  ...
  const ancillaryCostTotal = incidental;
  const total = price + incidental;
  const cash = price - paid - loan + incidental;
  return { price, firstHome, over85, loan, paid, legal, bond, rate, acquisitionBefore, reduction, acquisition, education, rural, acquisitionTotal, broker, brokerage, loanStamp, contractStamp: contractStamp(price), incidental, ancillaryCostTotal, total, cash };
}
